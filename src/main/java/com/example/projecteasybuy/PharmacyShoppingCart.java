...
public class PharmacyShoppingCart {
    private static PharmacyShoppingCart INSTANCE;
    Products product;

    public static PharmacyShoppingCart getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new PharmacyShoppingCart();
        }
    ...
    public double calculateTotal() {
        System.out.println(Arrays.asList(entries)); // method 1
        System.out.println(Collections.singletonList(entries)); // method 2
        double total = 0;
    ...
    public List<CartEntry> pharmacygetEntries() {
